...
Slideshow.prototype.initUI = function() {
	...
	for(var i = 0; i < this.imgs.length; i++) {
		var str = "<img src='" + this.imgs[i] + "' style=' width:" + this.width + "px;height:" + this.height + "px;position:absolute;left:0px;top:0px;z-index:" + (this.imgs.length - i) + ";display:none;'/>";
		$(this.boxId).append(str);
	}
	//2、让第一张图片显示
	$(this.boxId + " img:eq(0)").css({
		"display": "block"
	});
	//创建所有按钮
	...
	$("#listitem li:eq(0)").css({
		"background-color": this.btnHighColor
	});
	//上一个
...
Slideshow.prototype.changeBtnColor = function() {
	$("#listitem li:eq(" + (this.currInOrd - 1) + ")").css({
		"background-color": this.btnHighColor
	}).siblings().css({
